(function(global){
	...
	Complex.eval = function(str){
		...
		function decompose(str){var a = [""], on = 0; function newa(){if(a[on] != ""){on++;a.push("");}}for(var i=0;i<str.length;i++){var _a = check(str, i, Complex.operators, Complex.prototype); if(str[i] == ',' || str[i] == '(' || str[i] == ')' || _a){newa(); if(str[i] == ',' || str[i] == '(' || str[i] == ')'){a[on]+=(str[i]); }else{a[on]+=_a; i+=_a.length-1; } newa(); }else{a[on]+=(str[i]); } } return a;}
		var a = decompose(str);
		...
		function pSolve(arr){
			...
			lar(function(i){
				if(ar[i] in Complex){
					if(rh(ar[i+1]) && ar[i+1]){
						if(Array.isArray(ar[i])) ar[i] = Complex[ar[i]].apply(null, ar[i+1]);
						else ar[i] = Complex[ar[i]].apply(null, [ar[i+1]]);
						ar.splice(i+1, 1);
					}
					else if(ar[i+1] in Complex.operators){
						var t = Complex[ar[i]]((ar[i+3]));
						t = Complex.operators[ar[i+1]].apply(t, [(ar[i+2])]);
						ar[i] = t;ar.splice(i+1, 3);
					}
				}
			});
			lar(function(i){
